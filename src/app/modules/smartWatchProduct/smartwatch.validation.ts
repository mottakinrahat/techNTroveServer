...
const BasicInfoSchema = z.object({
  ...
  healthTracking: z
    .string()
    .nonempty({ message: "Health tracking is required" }),
  sports: z.string().nonempty({ message: "Sports is required" }),
  waterResistance: z
    .string()
    .nonempty({ message: "Water resistance is required" }),
  ...
  specialFeatures: z
    .array(z.string())
    .nonempty({ message: "Special features are required" }),
});

const ExteriorSchema = z.object({
  color: z.array(z.string()).nonempty({ message: "color is required" }),
});
...
const SmartWatchProductValidationSchema = z.object({
  brand: z.string(),
  productImage: z
    .array(productImageValidationSchema)
